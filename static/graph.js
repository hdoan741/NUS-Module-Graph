...
var listModuleChosen = new Array();
var listModuleCheck = new Array();
var graph;
var exc;
var directLink;
...
function checkOk (checked, req, listModuleChosen) {
    console.log("abc" + req + " | " + listModuleChosen + "sdfsdf" );
    if (req == null) return 1;
    if (! (req instanceof Array)) 
    {
        if (!arrayContain(listModuleChosen, req)) return 0;
        if (checked[req] == null)
            checked[req] = checkOk(checked, graph[req], listModuleChosen);
        return checked[req];
    }
    var type = req[0];
    if (type == 1) { // AND
        ...
        for (var i = 1; i < req.length; i++)
            ok = ok && checkOk(checked, req[i], listModuleChosen);
    } else {
        var ok = false;
        for (var i = 1; i < req.length; i++)
            ok = ok || checkOk(checked, req[i], listModuleChosen);
...
function checkAvailable(listModuleChosen) {
    ...
    for (var i = 0; i < listModuleChosen.length; i++) {
        ...
        if (checked[code] == null) 
            checked[code] = checkOk (checked, graph[code], listModuleChosen);
        console.log ("result " + checked[code]);
    }
    return checked;
}
...
function moduleOnLayerClicked() {
    ...
    currentLayer.remove(currentLayer.getChild("line group"));
    drawPreForModule(currentLayer, this.getName());
    stage.draw();
}

function moduleOnLayerTicked() {
    var string = this.getName();
    var code = string.substring(8, string.length);
    var moduleButton = currentLayer.getChild(code);
    console.log(moduleButton);
    if (this.image == imageTick) {
        console.log("image un tick");
        listModuleCheck[code] = false;
        this.image = imageNotTick;
        moduleButton.color = BUTTON_COLOR;
    } else {
        console.log("image tick");
        this.image = imageTick;
        listModuleCheck[code] = true;
    }

    var listModuleChosen = new Array();
    for (var i = 0; i < listModule.length; i++) {
        var code = listModule[i];
        if (listModuleCheck[code]) {
            listModuleChosen.push(code);
        }
    }
    console.log("list chosen  " + listModuleChosen);
    var checked = checkAvailable(listModuleChosen);
    console.log(checked);
    for (var i = 0; i < listModuleChosen.length; i++) {
        var code = listModuleChosen[i];
        var moduleButton = currentLayer.getChild(code);
        console.log("code " + code + " " + checked[code]);
        if (checked[code])
            moduleButton.color = ENABLE_COLOR;
        else 
            moduleButton.color = DISABLE_COLOR;
    }
...
function drawLevelPreReq(layer, posX, posY, maxX, name) {
    ...
    for (var i = 0; i < name.length; i++) { 
        (function(){
         ...
         var moduleTick = new Kinetic.Shape(
                  function() {
                           drawImage(this, posButtonX + 1, posButtonY + 1, 12, 12, imageTick);                                    
                  }, "checkbox" + moduleName 
         );
...
function bubble(levelList) {
    for (var i = 0; i < levelList.length; i++) {
        var nodeIndex = levelLink[i];
        var minNode = nodeIndex;
        for (var j = i; j < levelList.length; j++) {
            var nodeIndex1 = levelLink[j];
            if (currentLayer.getChild(minNode).origin.x > currentLayer.getChild(levelList[j]).origin.x)
                minNode = levelList[j];
        }
        if (minNode != nodeIndex) {
            var module1 = currentLayer.getChild(nodeIndex);
            var module2 = currentLayer.getChild(minNode);
            var tempOrg = module1.origin;
            module1.origin = module2.origin;
            module2.origin = tempOrg;
        }
    }
    stage.draw();
...
function drawPreForModule(layer, code) {
    ...
    var level1 = preLevel[code];
    var levelModule = new Array(7);
    for (var i = 0; i < 7 ; i++) levelModule = new Array();
    for (var i = 0; i < directLink.length; i++) {
        levelModule[preLevel[directLink[i]]].push(directLink[i]);
    }
    for (var i = 0; i < 7; i++) {
       // if (levelModule[i] != nulla)
       // bubble(levelModule[i]);
    }
    var button = currentLayer.getChild(code);
    console.log(button);
    var pos = button.origin;
...
function addModuleEventListener() {
    for (var i = 0; i < listModuleChosen.length; i++) {
        ...
        module.on("click", moduleOnLayerClicked);
        if (code === currentLayer.getName()) 
            continue;
        var moduleTick = currentLayer.getChild("checkbox" + code);
        
        console.log("checkbox" + code + " " + moduleTick);
        moduleTick.on("click", moduleOnLayerTicked);
    }
}
...
function drawModule(code) {
    ...
    if (layer == null) {
        ...
        currentLayer = layer;
        var moduleAtHopLevel = new Array(7);
        for (var i = 0; i < 7; i++)
        ...
        if (exc[code] == null) 
            ...
            x = SCREEN_WIDTH - (exc[code].length * (WIDTH + BUTTON_DISTANCE) 
            - BUTTON_DISTANCE);
            x = parseInt( x/ 2);
        } 
        var posX;
        ...
        listModuleCheck = new Object;
        for (var i = 0; i < listModuleChosen.length; i++) {
            var code = listModuleChosen[i];
            listModuleCheck[code] = true;
        }
        
        console.log(listModuleCheck);
